...
from app.schemas.camareiro import CamareiroCreate, CamareiroUpdate
from app.crud.base import apply_updates


def listar(db: Session):
    return db.query(models.Camareiro).all()


def buscar(db: Session, matricula):
    return db.query(models.Camareiro).filter(models.Camareiro.matricula == matricula).first()


def criar(db: Session, dados: CamareiroCreate):
    ...
    obj = models.Camareiro(**dados.model_dump())
    db.add(obj)
    db.commit()
    db.refresh(obj)
    return obj
...
def atualizar(db: Session, matricula, dados: CamareiroUpdate):
